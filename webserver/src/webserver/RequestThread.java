package webserver;

import java.io.*;
import java.net.*;
import java.util.*;

public class RequestThread extends Thread {
	
    public RequestThread(Socket socket, File rootDir) {
        _socket = socket;
        _rootDir = rootDir;
    }
    
    private static void sendHeader(BufferedOutputStream out, int code, long contentLength, long lastModified) throws IOException {
        
    	System.out.println("I am in header with : " +out+" "+code+" "+lastModified);
    	out.write(("HTTP/1.0 " + code + " OK\r\n" + 
                   "Date: " + new Date().toString() + "\r\n" +                  
                   ((contentLength != -1) ? "Content-Length: " + contentLength + "\r\n" : "") +
                   "Last-modified: " + new Date(lastModified).toString() + "\r\n" +
                   "\r\n").getBytes());
    }
    
    private static void sendError(BufferedOutputStream out, int code, String message) throws IOException {
        message = message + "\n";
        sendHeader(out, code, message.length(), System.currentTimeMillis());
        out.write(message.getBytes());
        out.flush();
        out.close();
    }
    
    public void run() {
        InputStream reader = null;
        try {
            _socket.setSoTimeout(30000);
            BufferedReader in = new BufferedReader(new InputStreamReader(_socket.getInputStream()));
            BufferedOutputStream out = new BufferedOutputStream(_socket.getOutputStream());
                        String request = in.readLine();
            
            if(SimpleWebServer.state != null && SimpleWebServer.state.equals("maintenance")) {
            	sendError(out, 503, "Server in maintenance.");
                return;
            }
//            
            if (request == null || !request.startsWith("GET ") || !(request.endsWith(" HTTP/1.0") || request.endsWith("HTTP/1.1"))) {
                // invalid request  
                sendError(out, 500, "Invalid Method.");
                return;
            }            
            String path = request.substring(4, request.length() - 9);            
            File file = new File(_rootDir, URLDecoder.decode(path, "UTF-8")).getCanonicalFile();
            
            if (file.isDirectory()) {
                // Check if index file present in the directory.
//                File indexFile = new File(file, "index.html");
                File indexFile = new File(file, "a.html");
                if (indexFile.exists() && !indexFile.isDirectory()) {
                    file = indexFile;
                }
            }

            if (!file.toString().startsWith(_rootDir.toString())) {
                // trying to acess unauth zone
                sendError(out, 403, "Permission Denied.");
            }
            else if (!file.exists()) {
                //  file not found.
                sendError(out, 404, "File Not Found.");
            }
            else if (file.isDirectory()) {
            	
            	if(SimpleWebServer.state.equals("maintenance")) {
                	sendError(out, 503, "Server in maintenance.");
                    return;
                }
              
                if (!path.endsWith("/")) {
                    path = path + "/";
                }
                File[] files = file.listFiles();
                sendHeader(out, 200, -1, System.currentTimeMillis());
                String title = "Index of " + path;
                out.write(("<html><head><title>" + title + "</title></head><body><h3>Index of " + path + "</h3><p>\n").getBytes());
                for (int i = 0; i < files.length; i++) {
                    file = files[i];
                    String filename = file.getName();
                    String description = "";
                    if (file.isDirectory()) {
                        description = "&lt;DIR&gt;";
                    }
                    out.write(("<a href=\"" + path + filename + "\">" + filename + "</a> " + description + "<br>\n").getBytes());
                }
                out.write(("</p><hr><p>" + "</p></body><html>").getBytes());
            }
            else {
                reader = new BufferedInputStream(new FileInputStream(file));
           
                sendHeader(out, 200, file.length(), file.lastModified());
                
                byte[] buffer = new byte[4096];
                int bytesRead;
                while ((bytesRead = reader.read(buffer)) != -1) {
                    out.write(buffer, 0, bytesRead);
                }
                reader.close();
            }
            out.flush();
            out.close();
        }
        catch (IOException e) {
            if (reader != null) {
                try {
                    reader.close();
                }
                catch (Exception e1) {
                    // Do nothing.
                }
            }
        }
    
    }
    
    private File _rootDir;
    private Socket _socket;
    
}